<template>
  <div class="order-right">
    <div class="order-content">
      ...
      <div class="orders">
        <table class="order-item" v-for="item in dataLists.records" :key="item.id">
          <thead>
            <tr>
              <th colspan="5">
          ...
          <tbody>
            <tr v-for="(items,index) in item.orderDetailList" :key="items.id">
              <td width="60%">
                <div class="typographic">
                  <img :src="items.imgUrl" style="width:100px;height:100px"/>
                  <a href="#" class="block-text">包邮 {{items.skuName}}} 新款</a>
      ...
      <div class="choose-order">
       <!-- 分页 -->
          <Pagination :pageOne="page" :pageSize="limit" :total="dataLists.total" :contioune="5" @changeClick="changeClick"></Pagination>
      </div>
    </div>
    <!--猜你喜欢-->
    <div class="like">
...
export default {
  ...
  methods:{
    dataList(){
      this.$api.default.getOrderLists(this.page,this.limit).then((res)=>{
        if(res.code==200){
