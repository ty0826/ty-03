          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd changepirce="0" @click="change(items, item.spuSaleAttrValueList)" style="cursor: pointer" :class="{ active: items.isChecked == 1 }" v-for="items in item.spuSaleAttrValueList" :key="items.id">{{ items.saleAttrValueName }}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="count" disabled="true" />
                <a href="javascript:" class="plus" @click="count++">+</a>
                <a href="javascript:" class="mins" @click="count <= 1 ? (count = 1) : count--">-</a>
              </div>
              <div class="add">
...
export default {
  ...
  mounted() {
    this.$store.dispatch('getDetail', this.$route.params.detailId)
  ...
  methods: {
    ...
    //点击添加购物车{ skuId }/{ skuNum }
    async addShopcar() {
      try {

        //成功
        await this.$store.dispatch('addShopcars', { skuId: this.$route.params.detailId, skuNum: this.count })
        //利用会话存储进行路由传参,转换成字符串形式
        sessionStorage.setItem('skuInfo',JSON.stringify(this.skuInfo))
        //路由跳转
        this.$router.push({path:'/AddCartSuccess',query:{skuNum:this.count}})
      } catch (error) {
        //失败
        alert(error.message)
      }
